...
describe('Fetch Answer Comments',()=>{
    ...
    it('should be able to fetch answer comments', async () => {
        ...
        await inMemoryAnswerCommentRepository.create( makeAnswerComment({ answerId: new UniqueEntityID('1')}))
        await inMemoryAnswerCommentRepository.create( makeAnswerComment({ answerId: new UniqueEntityID('1')}))
        await inMemoryAnswerCommentRepository.create( makeAnswerComment({ answerId: new UniqueEntityID('1')}))
      ...
      it('should be able to fetch pagination to recent answers', async () => {
            ...
            for(let i = 1; i<22 ; i++){
                await inMemoryAnswerCommentRepository.create(makeAnswerComment({ answerId: new UniqueEntityID('32')}))
            }
